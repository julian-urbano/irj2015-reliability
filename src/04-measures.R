...
if(!file.exists(measure.path) | !file.exists(original.path) | any(is.na(assumptions))) {
  stop("usage: Rscript src/03-measures.R <measure> <collection> <normal> <homoscedastic> <uncorrelated> <random>\n")
}
...
path <- file.path("scratch/03-measures", measure.name, collection.name, assumptionsToPath(assumptions))
