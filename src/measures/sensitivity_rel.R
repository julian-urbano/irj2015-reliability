...
sensitivity_rel.estimate <- function(X, n_t_, extrapolator)
{
  observations <- split.half(X, sensitivity_rel.actual)
...
sensitivity_rel.actual <- function(X_estimate, X_truth, maxErrorRate = .05)
{
  ...
  for(i in 1:(ncol(X_estimate)-1)) {
    for(j in (i+1):ncol(X_estimate)) {
      n <- n+1
      d_estimate[n] <- (means_estimate[i] - means_estimate[j]) / max(means_estimate[i], means_estimate[j])
  ...
  if(length(i) == 0) {
    ...
    return(min(1, d_estimate[i[length(i)]]))
  }
}
